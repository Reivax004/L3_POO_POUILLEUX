    ...
    private static void handleDiscardAPair(GameCommand command){
        ...
        for (int i = 0; i < hand.size(); i++) {
            ...
            for (int j = i + 1; j < hand.size(); j++) {
                ...
                else if (card1.getValue().equals(card2.getValue()) && card1.getSymbol().getColor().equals(card2.getSymbol().getColor()) && command.body().equals("null")){
                    if (cardsToRemove.size() == 0) {
    ...
    private static void handleLooser(GameCommand command){
        for (Card card : hand) {
            if (card.getValue().equals(Value.VALET) && card.getSymbol().getName().equals("Pique")) {
                System.out.println(playerId + " est le pouilleux, car il a le Valet de Pique !");
                return;
            }
        }
    }
